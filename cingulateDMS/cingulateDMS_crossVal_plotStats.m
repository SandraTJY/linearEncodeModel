...
% Example (dummy):
neuralEventSummary = readtable('ST_neuralEventSummary.csv', ...
    'Delimiter', ',', 'ReadVariableNames', true);

%% 1) Prepare output folder
outDir = fullfile(pwd, 'NeuronPlots', 'cingulateDMS');
if ~exist(outDir, 'dir')
    mkdir(outDir);
end
...
allObjs_vid = struct(sessionField, obj);
sessionFields = fieldnames(allObjs_vid);

% Save R2 and deltaR2 into allResults
allResults = struct('R2_full', [], ...
                    'R2_delta', [] ); %ncell x nsubsets
R2_threshold = 0.01; %goodness of fit threshold for plotting
for iObj = 1:numel(sessionFields)
    ...
    for m = 1:numel(modelFields)
        ...
        allResults.R2_full = [allResults.R2_full R2_full]; 
        % --- Subset ΔR² ---
        deltaR2 = [];
        subsetGroups = {};
        ...
        %% ---- Plot per-neuron results ONLY if model fit was good ----
        if R2_full < R2_full_threshold
            continue;
        else
    ...
    allResults.R2_delta = [allResults.R2_delta deltaR2]; 
