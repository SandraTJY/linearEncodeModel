...
base_dir = 'C:/Users/stan/Documents/GitHub/linearEncodeModel/cingulateDMS/2025-09-16_1_SAT035';
results = struct();

% List of events
events = {'stimulusOnsetTime','rewardTime'};
baseline_pre = struct('stimulusOnsetTime', 0.5 ,...
                       'rewardTime', 2.5 );
baseline_dur = 0.5;
event_dur = 1;
    ...
    for c = 1:length(cellNames)
        ...
        for e = 1:length(events)
            ...
            for t = 1:length(eventTimes)
                ...
                baselineWindow = [evTime-baseline_pre.(eventName), (evTime-baseline_pre.(eventName)+baseline_dur)];
                postWindow = [evTime, evTime+event_dur]; % I think the reward activity ramp out slightly later, so I put it as 1.5s for now
...
%% Save results
save('ST_neuralEventResponses.mat','results');
...
% Save nicely as CSV
writetable(summaryTable, 'ST_neuralEventSummary.csv');
